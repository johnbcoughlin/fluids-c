...
use galerkin_2d::unknowns::{Unknown, initialize_storage, communicate};
...
pub fn maxwell_2d<'grid, Fx>(grid: &Grid<Maxwell2D>,
        ...
        Fx: Fn(&Vector<f64>) -> EH,
{
    ...
    let mut storage: Vec<EHStorage> = initialize_storage(
        u_0, reference_element.n_p as i32, reference_element, grid, operators);
...
pub fn maxwell_2d_example() {
    ...
    let grid: Grid<Maxwell2D> = assemble_grid(
        &reference_element, &operators, &mesh, &boundary_condition);
}
